...
def run(opts):
    ...
    for epoch in range(0, 1000000, 30):
        ...
        if opts.variation_type == 'graph_size':
            ...
            for scale in scales:
                ...
                task_prop['val_dataset'] = "{}__size_{}_scale_{}_{}_{}_seed{}.pkl".format(opts.problem,
                                                                                          ...
                                                                                          task_prop['high'],
                                                                                          "validation", "4321")
                task_prop['insertion_heuristic_cost_file'] = \
                    "results_all/validation/SCALE_{}_{}-{}_valfarthest_insertion.pkl".format(task_prop['graph_size'],
                                                                                             int(task_prop['low']),
                                                                                             int(task_prop['high']))
                tasks_list.append(task_prop)

        elif opts.variation_type == 'distribution':
            for num_modes in [1, 2, 5]:
                ...
                task_prop['insertion_heuristic_cost_file'] = \
                    "results_all/validation/GRID_{}_modes_{}_valfarthest_insertion.pkl".format(task_prop['graph_size'],
                                                                                               task_prop['num_modes'])
                tasks_list.append(task_prop)
        ...
        for task in tasks_list:
            ...
            if str(task) not in val_dict:
                ...
                val_dataset = problem.make_dataset(
                    ...
                    filename=opts.val_dataset + opts.variation_type + '/' + opts.problem + '/' + task['val_dataset'],
                ...
                with open(task['insertion_heuristic_cost_file'], 'rb') as handle:
                    ...
                    insertion_heuristic_costs = [insertion_heuristic_costs[k] for k in
                                                 sorted(insertion_heuristic_costs)]
        ...
        for task in tasks_list:
            ...
            opt_gap_wrt_ins_heu_cur_task_updated = tune_and_validate(task,
                                                                     model_meta,
                                                                     baseline,
                                                                     epoch,
                                                                     val_dataset,
        ...
        avg_opt_gap_val = total_opt_gap_across_tasks / len(tasks_list)
        print("Avg total_opt_gap_across_tasks all tasks after fine tune ", avg_opt_gap_val)
        tune_sequence_opt_gaps_across_epochs.append(avg_opt_gap_val)

        for index, x in enumerate(tune_sequence_opt_gaps_across_epochs):
            print(index, x)

        if min_cost > avg_opt_gap_val:
            min_cost = avg_opt_gap_val
            min_index = epoch
        ...
        dict_val_epoch_vs_best_till_now[epoch]['opt_gap_wrt_ir_cur_epoch'] = avg_opt_gap_val
