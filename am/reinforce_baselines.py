...
class RolloutBaseline(Baseline):
    ...
    def __init__(self, model, problem, opts, task = -1, epoch=0, rollout_baseline_dataset=None, update_baseline = True):
        super(Baseline, self).__init__()
        ...
        #not in testing
        if(update_baseline==True):
            self._update_model(model, epoch, rollout_baseline_dataset)
        else:
    ...
    def wrap_dataset(self, dataset):
        print("Evaluating baseline on dataset... of size" , self.task)
