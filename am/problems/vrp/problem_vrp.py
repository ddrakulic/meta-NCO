...
class VRPDataset(Dataset):
    ...
    def __init__(self, filename=None, num_samples=1000000, offset=0, distribution=None, task=None):
        ...
        if filename is not None:
            ...
            if (task['variation_type'] == 'distribution'):
                self.data = self.generate_GM_vrp_data(num_samples, task['graph_size'], task['mode_center'], CAPACITIES)
    ...
    def generate_GM_vrp_data(self, dataset_size, tsp_size, mode_center, CAPACITIES):
        # mu_array = [0.2, 0.8]
        print("mu array ", mode_center)
        sigma = 0.05
        print("sigma ", sigma)
        dataset = []
        for i in range(dataset_size):
            sigma_array = [sigma]*len(mode_center)
            mode_center_array = [a_m for a_m in mode_center
                                 ]
            # print(mode_center_array)
            # print("sigma array ",sigma_array)

            loc, y_true = make_blobs(n_samples=tsp_size, cluster_std=sigma_array,
                                      centers=mode_center_array)
            # print("data  ", data)
            # data = data[:, ::-1]
            # print(data)
            ...
            data =   {
                'loc': loc,
                # Uniform 1 - 9, scaled by capacities
                'demand': (torch.FloatTensor(tsp_size).uniform_(0, 9).int() + 1).float() / CAPACITIES[
                    tsp_size],
