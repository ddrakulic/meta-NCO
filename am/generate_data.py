...
def generate_uniform_tsp_data(dataset_size, tsp_size,low=0, high = 1):
    return np.random.uniform(low, high, size=(dataset_size, tsp_size, 2)).tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--graph_sizes', type=int, nargs='+', default=[-1],
                        help="Sizes of problem instances (default 20, 50, 100)")
    parser.add_argument("-f", action='store_true', help="Set true to overwrite")
    parser.add_argument('--seed', type=int, default=1234, help="Random seed")
    parser.add_argument('--variation_type', type=str, default="None", help='Shift in size/dist etc.')
    parser.add_argument('--graph_size', type=int, default="None", help='Shift in size/dist etc.')

    parser.add_argument('--low', type=float, default=0, help='unif dist low')
    parser.add_argument('--high', type=float, default=1, help='unif dist high')
    ...
    for problem, distributions in problems.items():
        for distribution in distributions or [None]:
            ...
            problem_instance= load_problem(opts.problem)
            if problem == 'tsp' or problem =='cvrp':
                if(opts.variation_type == 'graph_size'):
                    task_prop = {'graph_size': opts.graph_size, 'low': 0, 'high': 1, 'dist': 'uniform',
                                 'variation_type': opts.variation_type}

                    dataset = problem_instance.make_dataset(num_samples=opts.dataset_size,
                                                   distribution=opts.data_distribution, task=task_prop)
                    ...
                    task_prop = {'graph_size': opts.graph_size, 'low': opts.low, 'high': opts.high, 'dist': 'uniform',
                                 'variation_type': opts.variation_type}
                    print(task_prop)
                    ...
                    filename = os.path.join(datadir, "{}_{}_size_{}_scale_{}_{}_{}_seed{}.pkl".format(
                        problem,
                        "_{}".format(distribution) if distribution is not None else "",
                        opts.graph_size, opts.low, opts.high,opts.name, opts.seed))
                    save_dataset(dataset, filename)
                    ...
                    for i in [1,2,3,4,5,6,7,8]:
                        ...
                        filename = os.path.join(datadir, "{}_{}_size_{}_distribution_{}_{}_seed{}.pkl".format(
                            problem,
                            "_{}".format(distribution) if distribution is not None else "",
                            opts.graph_size, str(num_modes),opts.name, opts.seed))
