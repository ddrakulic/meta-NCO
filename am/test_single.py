...
def run(opts):
    ...
    print(dict_val_epoch_vs_best_till_now)
    first_time = dict_val_epoch_vs_best_till_now[max(dict_val_epoch_vs_best_till_now)]['time_of_cur_epoch']
    first_time = datetime.strptime(first_time, '%Y-%m-%d %H:%M:%S')
    print(first_time)

    epoch_id = max (dict_val_epoch_vs_best_till_now)

    print("EPOCH AT ", epoch_id)

    time_at_epoch_id = dict_val_epoch_vs_best_till_now[epoch_id]['time_of_cur_epoch']
    time_at_epoch_id=datetime.strptime(time_at_epoch_id, '%Y-%m-%d %H:%M:%S')

    time_diff = time_at_epoch_id-first_time
    print("time diff", time_diff)
    time_diff_minutes = int((time_diff.total_seconds())/60)
    print("time diff minutes ", time_diff_minutes)
    best_epoch_till_now = dict_val_epoch_vs_best_till_now[epoch_id]['best_epoch_till_now']
    epoch  = best_epoch_till_now#opts.test_at_epoch_id

    # Pretty print the run args
    print("epoch id ", epoch_id)
    print("time at epoch ", time_at_epoch_id)
    print("*******time diff", time_diff_minutes)
    # Pretty print the run args
    pp.pprint(vars(opts))
    ...
    load_path = opts.load_path+ '/epoch_' +str(best_epoch_till_now)+'_Time'+dict_val_epoch_vs_best_till_now[best_epoch_till_now]['time_of_cur_epoch']+'.pt' if opts.load_path is not None else opts.resume
