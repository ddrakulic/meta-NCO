...
from supervisors import multi_supervisor
from plots.make_plots import generate_plots
from supervisors.handlers.gcn_tsp.multi import MultiHandler

warnings.filterwarnings("ignore", category=UserWarning)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='meta_gcn_tsp')
    parser.add_argument("--task_type", type=str, default="scale")
    parser.add_argument("--train_task_list", type=int, nargs='+', default=[10])
    parser.add_argument("--test_task_list", type=int, nargs='+', default=[20])
    ...
    parser.add_argument("--train_steps_per_epoch", type=int, default=500, help="Training steps per epoch")
    ...
    # log directories
    log_dir = f"{args.output_dir}/meta_nco/gcn_tsp/multi/{args.task_type}/{args.job_id}/"

    multi_handler = MultiHandler(args, config_net)
    supervisor = multi_supervisor.Supervisor(args, multi_handler, log_dir)
