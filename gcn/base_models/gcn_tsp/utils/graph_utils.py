...
def mean_tour_len_nodes(x_edges_values, bs_nodes):
    ...
    for batch_idx in range(y.shape[0]):
        ...
        for y_idx in range(y[batch_idx].shape[0] - 1):
            ...
            cur_tour_len += W_val[batch_idx][i][j]

        running_tour_len += W_val[batch_idx][j][0] # Add final connection to tour/cycle
        cur_tour_len += W_val[batch_idx][j][0]
        all_tour_lens.append(cur_tour_len)
    all_tour_lens = torch.tensor(all_tour_lens)
    if torch.cuda.is_available():
        all_tour_lens = all_tour_lens.cuda()

    return running_tour_len / y.shape[0], all_tour_lens
