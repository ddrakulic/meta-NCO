...
from base_models.gcn_tsp.utils.beamsearch import *
from base_models.gcn_tsp.utils.graph_utils import *
...
def beamsearch_tour_nodes(y_pred_edges, beam_size, batch_size, num_nodes, probs_type='raw', random_start=False):
    """
    ...
    # Perform beamsearch
    beamsearch = Beamsearch(beam_size, batch_size, num_nodes, probs_type, random_start)
    trans_probs = y.gather(1, beamsearch.get_current_state().to(y.device))
    ...
    ends = torch.zeros(batch_size, 1, dtype=torch.long, device=y.device)
...
def beamsearch_tour_nodes_shortest(y_pred_edges, x_edges_values, beam_size, batch_size, num_nodes,
    ...
    ends = torch.zeros(batch_size, 1, dtype=torch.long)
    ...
    for pos in range(1, beam_size):
        ends = pos * torch.ones(batch_size, 1, dtype=torch.long)  # New positions
