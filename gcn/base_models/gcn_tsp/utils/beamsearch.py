...
class Beamsearch(object):
    ...
    def __init__(self, beam_size, batch_size, num_nodes, probs_type='raw', random_start=False):
        """
        Args:
            beam_size: Beam size
        ...
        self.start_nodes = torch.zeros(batch_size, beam_size, dtype=torch.long)
        if random_start:
            # Random starting nodes
            self.start_nodes = torch.randint(0, num_nodes, (batch_size, beam_size), dtype=torch.long)
        if torch.cuda.is_available():
            self.start_nodes = self.start_nodes.cuda()

        # Mask for constructing valid hypothesis
        self.mask = torch.ones(batch_size, beam_size, num_nodes, dtype=torch.float)
        if torch.cuda.is_available():
            self.mask = self.mask.cuda()

        self.update_mask(self.start_nodes)  # Mask the starting node of the beam search
        # Score for each translation on the beam
        self.scores = torch.zeros(batch_size, beam_size, dtype=torch.float)
        if torch.cuda.is_available():
    ...
    def advance(self, trans_probs):
        ...
        if len(self.prev_Ks) > 0:
            ...
            if self.probs_type == 'raw':
                beam_lk[:, 1:] = torch.zeros(beam_lk[:, 1:].size(), dtype=torch.float)
            elif self.probs_type == 'logits':
                beam_lk[:, 1:] = -1e20 * torch.ones(beam_lk[:, 1:].size(), dtype=torch.float)
        # Multiply by mask
    ...
    def update_mask(self, new_nodes):
        ...
        """
        arr = (torch.arange(0, self.num_nodes).unsqueeze(0).unsqueeze(1).expand_as(self.mask).type(torch.long))
        new_nodes = new_nodes.unsqueeze(2).expand_as(self.mask)
        if torch.cuda.is_available():
            arr = arr.cuda()
        update_mask = 1 - torch.eq(arr, new_nodes).type(torch.float)
    ...
    def get_hypothesis(self, k):
        ...
        hyp = -1 * torch.ones(self.batch_size, self.num_nodes, dtype=torch.long)
        if torch.cuda.is_available():
            hyp = hyp.cuda()
            k = k.cuda()
